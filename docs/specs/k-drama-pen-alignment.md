...
- **`packages/design-tokens/src/tokens.css`를 수정하지 않는다.** 다른 세션이
  ai-communication 작업으로 같은 파일을 동시에 수정 중이라 충돌한다. 사용자
  결정에 따라 공유 토큰에 새 항목을 올리는 대신 앱 경계 변수 레이어로 우회한다.
- 기존 공유 토큰으로 표현 가능한 값은 그 토큰을 그대로 쓴다.
- 기존 토큰에 대응이 없는 k-drama 전용 값은 `apps/k-drama/src/styles.css`의 앱
  경계 CSS 변수 레이어(`--kd-*` 네임스페이스)에 한 번만 정의하고, 선택자에서는
  그 변수를 참조한다. 생값이 선택자에 직접 나타나지 않게 하는 것이 목표다.
- 앱 경계 변수는 가능한 한 공유 토큰에서 파생시킨다(예:
  `--kd-progress-accent: var(--color-danger)`). 파생이 불가능해 리터럴을 두어야
  하면, 그 변수 정의 옆에 왜 공유 토큰으로 표현할 수 없는지 주석으로 남긴다.
- 후속 작업이 공유 토큰으로 승격할 수 있도록, 승격 후보를 매핑 문서에 기록한다.
