...
가로는 이미 맞춰져 있었고, 실제로 큰 문제는 **기기 세로 높이에 대응하는 값이 없다는 것**이었다. 히어로가 쓰는 화면 수를 재보면 기기가 짧을수록 나빠진다.

| 앱               | iPhone SE 320×568     | iPhone 12 390×844 | 15 Pro Max 430×932 |
| ---------------- | --------------------- | ----------------- | ------------------ |
| k-drama          | 1568px = **2.76화면** | 1657px = 1.96화면 | 1694px = 1.82화면  |
| ai-communication | 1538px = **2.71화면** | 1436px = 1.70화면 | 1392px = 1.49화면  |
| k-culture        | 1694px = **2.98화면** | 1650px = 1.95화면 | 1621px = 1.74화면  |

원인은 셋이다.

1. **히어로 미디어가 904 → 1044 → 1124px.** 기기가 커질수록 커진다. 카드 크기가 폭에 묶여 있어 화면이 길어져도 첫 화면에 들어오는 내용은 그대로다. 320×568에서 이것만 1.9화면이다.
2. **`vh` 단위.** `min-block-size: 100vh`와 `min(88vh, 780px)`는 모바일에서 URL 바가 숨은 큰 뷰포트를 기준으로 삼아 실제 보이는 높이보다 크다. 바가 접힐 때 레이아웃이 튄다.
3. **ai-communication은 320×568에서 히어로 CTA 하단이 594px**로 첫 화면을 넘는다.

`400px`를 넘는 고정 높이가 CSS에 26곳 있지만 **폰 폭에서는 적용되지 않는다.** 320px에서 재보면 전부 `min-height: 0`으로 계산되고, 화면보다 높은 블록은 모두 텍스트가 든 내용 기반 스택이다. 상한을 씌우면 본문이 잘리므로 이 경로는 버렸다.

## 타깃 (target)

- 디바이스: 모바일 우선. 보장 범위는 폭과 높이를 함께 갖는 실제 기기 뷰포트다 — 320×568 · 360×640 · 390×844 · 412×915 · 430×932 · 820×1180. 280px(폴드 커버)는 보장하지 않는다.
...
- 포함(scope_in)
  ...
  - 폰에서 히어로 미디어를 세로 스택에서 가로 레일(scroll-snap)로. 카드 수와 순서는 그대로 두고 세로 비용만 한 장 높이로 줄인다. 스크롤 영역은 키보드로 닿아야 하므로 `tabindex`와 이름을 준다.
  - 화면을 채워야 하는 블록의 `vh`를 `svh`로. iOS 15.0~15.3을 위해 `vh` 선언을 앞에 남긴다.
  - 세로가 짧은 화면(`max-height: 40rem`)에서 히어로의 여백과 타입 스케일을 낮춰 CTA를 첫 화면에 둔다.
...
6. 보장 뷰포트 전부에서 히어로가 쓰는 화면 수가 2화면 이하다.
7. 보장 뷰포트 전부에서 히어로의 주 CTA 하단이 첫 화면 안에 있다.
8. `pnpm build`, `pnpm typecheck`, `pnpm lint`, `pnpm test`, `pnpm e2e`가 모두 통과하고 axe critical/serious 위반이 0건으로 유지된다.
