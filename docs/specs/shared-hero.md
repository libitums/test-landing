...
이 문서와 `packages/contracts/src/landing.ts`의 `HeroCtaContent`, `HeroContent`, `HeroProps`, `landingTestIds.hero`, `landingTestIds.heroCta`, `landingTestIds.heroMedia`를 구현·디자인·테스트 레이어의 고정 계약으로 선언한다. Hero에 링크·콜백·이벤트를 재도입하거나 미디어를 전용 prop으로 바꾸려면 이 계약을 먼저 갱신해야 한다.
