...
현재 `apps/ai-communication/src/app/EarlyAccessPage.tsx`의 폼, 성공 및 오류 상태 UI. 시각 변경 없음.
...
응답 타입과 상태 매핑의 단일 출처는 각각 `EarlyAccessRegistrationResponse`와 `earlyAccessHttpStatus`다. CORS 허용 origin은 서버 allowlist와 일치시키며 preflight는 rate-limit 횟수에 포함하지 않는다.
...
| Edge Function              | `EARLY_ACCESS_ALLOWED_ORIGINS`  | 서버 전용 설정 | 쉼표로 구분한 정확한 origin allowlist. wildcard는 허용하지 않는다.                       |

함수 이름, 프로젝트/테이블 매핑, 5회/60초 값은 빌드 계약 상수이며 환경변수로 바꾸지 않는다. 로컬/preview/production은 각각 별도 `.env` 또는 플랫폼 secret 값을 제공하며 실제 값은 commit하지 않는다. 저장소에는 이름만 든 `.env.example`을 둘 수 있다.

## 프론트엔드 port 계약

기존 `EarlyAccessPage`와의 호환 경계는 다음과 같다.
