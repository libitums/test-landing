...
인라인 텍스트 링크로 표현한다. 링크 색, 타입, 밑줄, 상태와 포커스에 필요한 공통
토큰이 모두 있으므로 CTA 전용 토큰은 추가하지 않는다.
...
| Early Access CTA 기본 / hover / active    | `--color-fg` / `--color-accent` / `--color-accent-active` |
...
- 각 `SharedFeatureContent`의 앱별 자식에는 하나의 Early Access 인라인 텍스트 링크를
  ...
  목적지만 소유한다.
- 기본 상태는 투명한 표면과 경계 없는 `--color-fg` 텍스트를 쓰며 타입은
  `--text-base` / `--font-semibold` / `--leading-body`다. 링크는 기본 상태부터
  밑줄을 표시해 색상 없이도 상호작용 가능함을 알린다. 버튼형 패딩, 최소 블록 크기,
  radius, 고정 너비는 적용하지 않는다.
- hover는 `--color-accent`, active는 `--color-accent-active`로 바꾸되 밑줄을 계속
  유지한다. hover와 active 모두 색상만으로 구분하지 않도록 밑줄의 시각적 강조도
  함께 유지하며, 상태 전환이 레이아웃을 움직이지 않아야 한다.
...
- 기본, hover, active 글자 토큰은 `--color-bg`와 `--color-surface-soft` 양쪽에서
  일반 텍스트 WCAG AA를 만족해야 한다. 밑줄은 모든 표면에서 남아 색 차이가 유일한
  링크 단서가 되지 않는다.
