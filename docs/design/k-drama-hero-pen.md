...
- CTA 노드 `jMjT5`와 proof 노드 `dlClE`는 렌더링하지 않으며 빈 자리도 남기지
  않는다.
...
| 토큰                       | 용도                    | 도입 이유                                                                      |
| -------------------------- | ----------------------- | ------------------------------------------------------------------------------ |
| `--color-canvas-warm`      | Hero 배경               | 기존 `--color-surface-soft`와 원본 canvas 색이 달라 정확한 배경 역할을 분리    |
...
`--color-editorial-muted-fg`, `--space-*`, `--content-max`를 재사용한다.

## 컴포넌트별 시각 스펙

### Hero surface와 copy

- surface는 `--color-canvas-warm`이며 최소 높이를 고정하지 않는다. Pencil의
...
- copy group은 `--content-hero-copy`를 넘지 않고 중앙 정렬한다. label,
  headline, subhead는 문서 순서와 시각 순서가 같다.
...
- copy 아래 한 행에 compact → wide → compact 순으로 배치하고 그룹 전체를
  가운데 정렬한다. DOM 순서도 동일하게 유지한다.
...
- `--breakpoint-mobile` 이하에서 label → headline → subhead → visuals의 순서는
  유지한다.
...
- shadow나 radius를 지원하지 않아도 label, headline, subhead와 세 visual의
  문서 순서 및 읽을 수 있는 내용은 남아야 한다.
- 모션은 도입하지 않으므로 reduced-motion 분기는 필요하지 않다.

## 대비 확인

- `--color-editorial-fg` (`#111111`) / `--color-canvas-warm` (`#fcfaf7`):
  약 **18.1:1**, WCAG AA 통과.
- `--color-editorial-muted-fg` (`#5f6368`) / `--color-canvas-warm`
  (`#fcfaf7`): 약 **5.8:1**, 일반 텍스트 WCAG AA 통과.
- card 내부 텍스트는 이미지 위에 직접 배치하지 않는다. 부득이한 overlay는
  불투명 scrim과 함께 각 상태에서 4.5:1 이상을 별도로 검증한다.

## 구현·리뷰 검증 기준

1. K-drama Hero에 label, headline, subhead, 세 visual만 보이고 CTA/proof 및
   그 빈 간격이 없다.
...
6. label/headline/subhead의 두 색상 조합이 WCAG AA를 만족한다.

## 디자인 고정

위 토큰과 시각 스펙을 K-drama Pencil Hero의 디자인 단일 출처로 고정한다.
구현이 CTA, proof, 카드 순서, 반응형 재배치를 바꾸려면 이 문서를 먼저 갱신해야
한다.
